...
1. `AGENTS.md`
2. `docs/SUPABASE_DATA_CONNECTIONS.md`
3. `app/src/types/database.ts`
...
- DB 스키마를 바꾸면 migration, `app/src/types/database.ts`, query `select(...)`, server action payload, RLS 정책을 함께 확인합니다.
...
3. `app/src/types/database.ts`가 실제 스키마와 맞는지 확인합니다.
4. query 파일과 action 파일을 함께 봅니다.
   - 조회: `app/src/lib/queries/*`
   - 쓰기: `app/src/lib/actions/*`
5. 화면 진입점도 확인합니다.
...
9. 변경 내용이 데이터 연결 구조에 영향을 주면 `docs/SUPABASE_DATA_CONNECTIONS.md`와 이 문서를 업데이트합니다.

## 현재 특히 조심할 부분

- `app/src/types/database.ts`는 수동 관리라 실제 DB와 drift가 생기기 쉽습니다.
...
- 이적 탭 UI와 이적 수정/복귀 클라이언트 로직은 `app/src/app/admin/AdminTransfersPanel.tsx`를 먼저 확인합니다.
- `app/src/app/admin/AdminDashboard.tsx`는 전체 섹션 라우팅과 선수/투표/구단 폼 중심으로 유지합니다.
- 큰 관리자 파일을 수정할 때는 필요한 섹션 컴포넌트만 읽고, 전체 파일 출력은 피합니다.

투표:

- 조회: `app/src/lib/queries/polls.ts`
- 투표 제출: `app/src/lib/actions/vote.ts`
- 투표 가능 여부: `app/src/lib/polls/vote-eligibility.ts`
- 댓글/좋아요: `app/src/lib/queries/comments.ts`, `app/src/lib/actions/comments.ts`
- 화면: `app/src/app/page.tsx`, `app/src/app/polls/[id]/page.tsx`

인증/온보딩/마이페이지:

- 로그인: `app/src/app/login/LoginPageClient.tsx`
- OAuth callback: `app/src/app/auth/callback/route.ts`
- middleware: `app/src/middleware.ts`
- 닉네임 저장: `app/src/lib/actions/onboarding.ts`
- 마이페이지: `app/src/app/my/page.tsx`

구단/선수:

- 조회: `app/src/lib/queries/club.ts`
- 선수 댓글: `app/src/lib/actions/player-comments.ts`
- 화면: `app/src/app/club/page.tsx`, `app/src/app/players/[id]/page.tsx`

이적/작별:

- 조회: `app/src/lib/queries/farewells.ts`
- 쓰기: `app/src/lib/actions/farewells.ts`
- 화면: `app/src/app/transfers/page.tsx`, `app/src/app/farewells/[id]/page.tsx`

관리자:

- 화면: `app/src/app/admin/page.tsx`, `app/src/app/admin/AdminDashboard.tsx`
- action: `app/src/lib/actions/admin.ts`, `app/src/lib/actions/farewells.ts`
- 권한 판정: `app/src/lib/admin.ts`
- service-role client: `app/src/lib/supabase/admin.ts`
...
- 새 테이블/컬럼/버킷을 추가하면 `docs/SUPABASE_DATA_CONNECTIONS.md`에 먼저 반영합니다.
