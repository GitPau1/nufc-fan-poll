-- Restore overall rating poll responses and comment likes.
...
DROP POLICY IF EXISTS "rating_votes: public read" ON public.rating_votes;
CREATE POLICY "rating_votes: public read"
  ON public.rating_votes FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "rating_votes: insert authenticated" ON public.rating_votes;
CREATE POLICY "rating_votes: insert authenticated"
  ON public.rating_votes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "rating_vote_likes: public read" ON public.rating_vote_likes;
CREATE POLICY "rating_vote_likes: public read"
  ON public.rating_vote_likes FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "rating_vote_likes: insert authenticated" ON public.rating_vote_likes;
CREATE POLICY "rating_vote_likes: insert authenticated"
  ON public.rating_vote_likes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "rating_vote_likes: delete own" ON public.rating_vote_likes;
